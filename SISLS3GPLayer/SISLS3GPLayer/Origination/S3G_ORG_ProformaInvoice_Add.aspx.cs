...
public partial class Origination_S3G_ORG_ProformaInvoice_Add : ApplyThemeForProject
{
    ...
    static string strPageName = "Proforma Invoice";
    ...
    string strRedirectPageView = "window.location.href='../Origination/S3GORGPIVITransLander.aspx?Code=PRFINV';";
    string strRedirectPageAdd = "window.location.href='../Origination/S3G_ORG_ProformaInvoice_Add.aspx?qsMode=C';";
    ...
    public static Origination_S3G_ORG_ProformaInvoice_Add obj_Page;
    ...
    protected void gvDlivery_RowEditing(object sender, GridViewEditEventArgs e)
    {
        try
        {
            ...
            txtPIQuantity.SetDecimalPrefixSuffix(10, 0, true, false, "PI Quantity");
            ...
            txtVAT1.SetUserDecimalPrefixSuffix(3, 2, false, "VAT Percentage");
    ...
    protected void gvDlivery_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        try
        {
            ...
            UserControls_S3GAutoSuggest txtBillingStateIT = (UserControls_S3GAutoSuggest)grvRow.FindControl("txtBillingStateIT");
            UserControls_S3GAutoSuggest txtBillingBranchIT = (UserControls_S3GAutoSuggest)grvRow.FindControl("txtBillingBranchIT");
            Label lblAssetClassID = (Label)grvRow.FindControl("lblAssetClassID");
            ...
            if (Utility.StringToDate(txtDate.Text) >= Utility.StringToDate(GSTEffectiveDate.Text))
            {
                ...
                //    dictParam.Add("@Company_ID", intCompanyId.ToString());
                //    dictParam.Add("@Class_ID", lblAssetClassID.Text);
            ...
            dtPO = new DataTable();
            dtPO.Columns.Add("PI_det_ID");
            dtPO.Columns.Add("PO_DTL_ID");
            dtPO.Columns.Add("PO_HDR_ID");
            dtPO.Columns.Add("PI_No");
            dtPO.Columns.Add("PI_Date");
            dtPO.Columns.Add("PI_Quantity");
            ...
            row["PI_det_ID"] = lblPI_det_ID.Text;
            row["PO_DTL_ID"] = lblPO_DTL_ID.Text;
            row["PO_HDR_ID"] = lblPO_HDR_ID.Text;
            row["PI_Date"] = Utility.StringToDate(txtPIDate.Text).ToString();
            row["PI_Quantity"] = txtPIQuantity.Text;
            row["PI_No"] = txtPINo.Text;
            ...
            intErrCode = objLoanAdmin_MasterClient.FunPubUpdatePI(SerMode, ObjDeliveryInsDataTable);

            if (intErrCode == 10)
            {
                Utility.FunShowAlertMsg(this.Page, "PI Quantity exceeding PO Quantity");
                return;
            }

            if (intErrCode == 11)
            {
                Utility.FunShowAlertMsg(this.Page, "PI Date should be Greater Than or Equal to PO Date");
                return;
            }

            if (intErrCode == 12)
            {
                Utility.FunShowAlertMsg(this.Page, "PI Number Already Exists");
                return;
            }

            if (intErrCode == 13)
            {
                Utility.FunShowAlertMsg(this.Page, "PI Amount exceeding PO Amount");
                return;
            }
    ...
    private void FunPriGetPIDetails()
    {
        try
        {
            ...
            Dictionary<string, string> dictParam = new Dictionary<string, string>();
            dictParam.Add("@Company_ID", intCompanyId.ToString());
            dictParam.Add("@PI_Hdr_ID", intPI_Hdr_ID.ToString());
            if (Session["PO_Number"] != null)
                dictParam.Add("@PO_Number", Session["PO_Number"].ToString());
            if (Session["PI_Number"] != null)
                dictParam.Add("@PI_Number", Session["PI_Number"].ToString());
            dictParam.Add("@User_ID", intUserId.ToString());

            dtDeliveryDetails = Utility.GetDefaultData("S3G_ORG_Get_PI_Details", dictParam);

            txtPINo.Text = dtDeliveryDetails.Rows[0]["PI_Number"].ToString();
            txtDate.Text = dtDeliveryDetails.Rows[0]["PO_Date"].ToString();
            txtPIDate.Text = dtDeliveryDetails.Rows[0]["PI_Date"].ToString();
    ...
    private void FunPriBindGrid()
    {
        ...
        gvDlivery.BindGridView("S3G_Org_Get_PI_Temp", dictParam, out intTotalRecords, ObjPaging, out bIsNewRow);
    ...
    private void FunPriBindPopUpGrid()
    {
        ...
        ucPopUpPaging.Navigation(intTotalRecords, ProPageNumRW, ProAssGrdPageSizeRW);
    ...
    private void FunPriGetTempPIGridDetails()
    {
        try
        {
            ...
            dictParam.Add("@PI_Number", txtPINo.Text);
            dictParam.Add("@Option", "1");
            DataTable dtInvoice = new DataTable();
            dtInvoice = Utility.GetDefaultData("S3G_Org_Get_Invoice_Grid", dictParam);
    ...
    private void FunPriGetPIGridDetails(int intOption)
    {
        try
        {
            ...
            gvDlivery.BindGridView("S3G_Org_Get_PI_Paging", dictParam, out intTotalRecords, ObjPaging, out bIsNewRow);
    ...
    private void FunPriDisableControls(int intModeID)
    {
        ...
        switch (intModeID)
        {
            ...
            case -1:// Query Mode
                ...
                btnGetLOV.Enabled = false;
                CalendarExtenderPIDate.Enabled = false;
                gvDlivery.Columns[72].Visible = false;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            intErrCode = objLoanAdmin_MasterClient.FunPubSavePI(out strLSQ_No, SerMode, ObjDeliveryInsDataTable);
            ...
            if (intErrCode == 0)
            {
                ...
                {
                    strKey = "Modify";
                    strAlert = strAlert.Replace("__ALERT__", "Proforma Invoice Updated Successfully");
                ...
                else
                {
                    ...
                    {
                        strAlert = "Proforma Invoice " + strLSQ_No + " details added successfully";
            ...
            {
                Utility.FunShowAlertMsg(this.Page, "Document sequence number is not defined for Proforma Invoice");
                strRedirectPageView = "";
            }
    ...
    {
        Session["PI_Number"] = null;
        Session["PO_Number"] = null;
        // wf cancel
        if (PageMode == PageModes.WorkFlow)
            ReturnToWFHome();
        else
            Response.Redirect("../Origination/S3GORGPIVITransLander.aspx?Code=PRFINV");
    }
    #endregion
    ...
    protected void btnDICancel_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            int intResult = objLoanAdmin_MasterClient.FunPubCancelPI(out ErrorCode, intPI_Hdr_ID, txtPINo.Text, ddlPONo.SelectedText);
            if (intResult == 0)
            {
                string strRedi = "../Origination/S3GORGPIVITransLander.aspx?Code=PRFINV";
                Utility.FunShowAlertMsg(this.Page, "Proforma Invoice Cancelled successfully", strRedi);
                Session["PI_Number"] = null;
                Session["PO_Number"] = null;
                return;
            }
            if (intResult == 5)
            {
                Utility.FunShowAlertMsg(this.Page, "Proforma Invoice already mapped with RS, Unable to cancel.");
                btnDICancel.Enabled = false;
            }
            if (intResult == 6)
            {
        ...
        catch (Exception objException)
        {
            ...
            cvDelivery.ErrorMessage = "Unable to cancel Proforma Invoice ";
            cvDelivery.IsValid = false;
        }
        finally
        {
...
//protected void txtPIDate_TextChanged(object sender, EventArgs e)
//{
//    int isEffective = 0;
...
//    if (Utility.StringToDate(txtPIDate.Text) < Utility.StringToDate(GSTEffectiveDate.Text))
...
//    {
//        Utility.FunShowAlertMsg(this, "Invoice Number is inappropriate for the selected Date. Please Clear/Cancel");
//        txtPIDate.Text = LPODate.Text;
//    }
//    LPODate.Text = txtPIDate.Text;
//}
