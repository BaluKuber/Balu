...
/// Purpose             :   This is the Report screen for searching Asset Insurance Status Details
/// Last Updated By		:   
/// Last Updated Date   :   
/// <Program Summary>
...
public partial class Reports_S3G_RPT_AssetInsuranceStatusReport : ApplyThemeForProject
{
    UserInfo ObjUserInfo = new UserInfo();
    S3GSession ObjS3GSession = new S3GSession();
    PagingValues ObjPaging = new PagingValues();
    public static Reports_S3G_RPT_AssetInsuranceStatusReport obj_Page;
        ...
        try
        {
            ...
            if ((!string.IsNullOrEmpty(txtDateFrom.Text)) && (!string.IsNullOrEmpty(txtDateTo.Text)))
            {
                if ((Utility.StringToDate(txtDateFrom.Text)) > (Utility.StringToDate(txtDateTo.Text)))
                {
                    ...
                    {
                        Utility.FunShowAlertMsg(this, "RS Activation From Date should be lesser than the RS Activation To Date");
                    }
                    else
                    {
                        Utility.FunShowAlertMsg(this, "RS Activation To Date should be greater than the RS Activation From Date");
                    }
                    return;
                }
            }
    ...
    public void BindAssetInsuranceDetailsReport()
    {
        try
        {
           ...
           // divAssetInsuranceStatus.Visible = true;
            ...
            grvAssetInsuranceStatusReport.BindGridView("[S3G_RPT_AssetInsuranceStatusReport]", Procparam, out intTotalRecords, ObjPaging, out blnIsNewRow);
            ...
            if (blnIsNewRow)
            {
/*added by vinodha m on sep 25,2015 - added to disable export button when grid have no records*/                
...
/*added by vinodha m on sep 25,2015 - added to disable export button when grid have no records*/                
    ...
    public void ExcelExport()
    {
        try
        {
            ...
            dtGetdata = Utility.GetDefaultData("[S3G_RPT_AssetInsuranceStatusReport]", Procparam);
            ...
            if (Grv.Rows.Count > 0)
            {
                ...
                string attachment = "attachment; filename=AssetInsuranceStatus_Report.xls";
                ...
                row = dtHeader.NewRow();
                row["Column1"] = "Asset Insurance Status Deal Wise Report";
                dtHeader.Rows.Add(row);

                row = dtHeader.NewRow();
                if (txtDateFrom.Text == string.Empty)
                    row["Column1"] = "RS Activation From Date : ";
                else
                    row["Column1"] = "RS Activation From Date : " + txtDateFrom.Text;

                if (txtDateTo.Text == string.Empty)
                    row["Column2"] = "RS Activation To Date : ";
                else
                    row["Column2"] = "RS Activation To Date : " + txtDateTo.Text;
                dtHeader.Rows.Add(row);
