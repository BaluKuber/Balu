...
public partial class Reports_S3GRptWCTReport : ApplyThemeForProject
{
    ...
    string strPageName = "WCT Report";
    public string strDateFormat;
    ReportAccountsMgtServicesClient objSerClient;
    public static Reports_S3GRptWCTReport obj_Page;
    ...
    private void FunPriLoadPage()
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                pnlWCTReportDetails.Style.Add("display", "none");
            }
        }
        catch (Exception ex)
        {
    ...
    private void FunPriBindGrid(int IS_EXPORT)
    {
        ...
        try
        {
            ...
            if (ddlVendorName.SelectedValue != "0" && ddlLesseeName.SelectedText != String.Empty)
            ...
            else if (ddlVendorName.SelectedValue != "0" && Convert.ToInt32(ddlVendorName.SelectedValue) > 0 && ddlVendorName.SelectedText == String.Empty)
                Procparam.Add("@Entity_ID", Convert.ToString("0"));
            else
                Procparam.Add("@Entity_ID", Convert.ToString("-1"));

            if (ddlState.SelectedValue != "0" && ddlState.SelectedText != String.Empty)
                Procparam.Add("@State_ID", Convert.ToString(ddlState.SelectedValue));

            else if (ddlState.SelectedValue == "0" && ddlState.SelectedText == String.Empty)
                Procparam.Add("@State_ID", Convert.ToString("0"));
            else if (ddlState.SelectedValue != "0" && Convert.ToInt32(ddlState.SelectedValue) > 0 && ddlState.SelectedText == String.Empty)
                Procparam.Add("@State_ID", Convert.ToString("0"));
            else
                Procparam.Add("@State_ID", Convert.ToString("-1"));
            ...
            pnlWCTReportDetails.Style.Add("display", "block");

            grvWCTReportDetails.BindGridView("S3G_RPT_Get_WCTReport_Paging", Procparam, out intTotalRecords, ObjPaging, out bIsNewRow);

            //This is to hide first row if grid is empty
            if (bIsNewRow)
            {
                grvWCTReportDetails.Rows[0].Visible = btnExport.Visible = false;
            }
            else
                btnExport.Visible = true;

            grvWCTReportDetails.Visible = ucCustomPaging.Visible = true;
    ...
    protected void FunProExport(GridView Grv, string FileName)
    {
        try
        {
            ...
            if (Grv.Rows.Count > 0)
            {
                ...
                row = dtHeader.NewRow();
                row["Column1"] = "WCT Report";
                dtHeader.Rows.Add(row);

                row = dtHeader.NewRow();

                if (ddlLOB.SelectedValue == "0")
                    row["Column1"] = "Line of Business : All";
                else
                ...
                if (ddlState.SelectedValue == "0")
                    ...
                    row["Column2"] = "State : " + ddlState.SelectedText;
                dtHeader.Rows.Add(row);

                row = dtHeader.NewRow();
                row["Column1"] = " Start Date " + txtStartDate.Text ;
                row["Column2"] = " End Date " + txtEndDate.Text;
                dtHeader.Rows.Add(row);                                
    ...
    protected void btnClear_Click(Object sender, EventArgs e)
    {
        ...
        grvWCTReportDetails.Visible = ucCustomPaging.Visible = btnExport.Visible = false;
        pnlWCTReportDetails.Style.Add("display", "none");
    }
    ...
    {
        FunPriBindGrid(1);
        FunProExport(grvWCTReportDetails, "WCT Report");
    }
    ...
    protected void grvWCTReportDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
