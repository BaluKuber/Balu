...
public partial class LoanAdmin_S3GDebitCreditNote_View : ApplyThemeForProject
{
    ...
    string strRedirectCreatePage = "";
    string DownFile = "";
    string strScipt1 = "";
    string Stock_Hdr_Id = "";
    int nDigi_Flag = 0;
    string SignedFile = "";
    // page to redirect to the page in Create mode
    ...
    public static LoanAdmin_S3GDebitCreditNote_View obj_Page;
    ...
    protected void grvTransLander_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)                // If data Row then check the data type and set the style - Alignment.
        {
            ...
            if (ProgramCode == strCRDRNote)
            {
                if (e.Row.Cells[15].Text.ToUpper() != "PENDING")
                {
                    imgbtnEdit.Enabled = false;
                    imgbtnEdit.CssClass = "styleGridEditDisabled";
                }
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                Utility.FunShowAlertMsg(this, "Select the Transaction Type");
                return;
            }
            ...
            foreach (GridViewRow grv in grvTransLander.Rows)
            {
                if (grv.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (chkSelect.Checked)
                    {
                        ...
                        Stock_Hdr_Id = imgQuery.CommandArgument;
                    }
                }
            }

            if (ddlType.SelectedValue == "1")
                strHTML = PDFPageSetup.FunPubGetTemplateContent(intCompanyID, 3
                    , 0, 21, Stock_Hdr_Id);
            ...
            if (grvTransLander.Rows.Count > 0)
            {
                    ...
                    foreach (GridViewRow grv in grvTransLander.Rows)
                    {
                        if (grv.RowType == DataControlRowType.DataRow)
                        {
                            ...
                            if (chkSelect.Checked)
                            {
                                ...
                                string FileName = PDFPageSetup.FunPubGetFileName(Convert.ToString(Stock_Hdr_Id) + intUserID + DateTime.Now.ToString("ddMMMyyyyHHmmss"));
                                ...
                                SaveDocument(strHTML, Convert.ToString(Stock_Hdr_Id), FilePath, FileName);
                            }
                        }
                    }
                ...
                string filePath = Server.MapPath(".") + "\\PDF Files\\DCN\\Signed.zip";
                //if (nDigi_Flag == 1)
                //{
                ...
                //else
                //{
                //    strScipt1 = "window.open('../Common/S3GDownloadPage.aspx?qsFileName=" + filePath.Replace("\\", "/") + "&qsHelp=Yes', 'newwindow','toolbar=no,location=no,menubar=no,width=900,height=600,resizable=yes,scrollbars=no,top=50,left=50');";
                //    ScriptManager.RegisterStartupScript(this, this.GetType(), "PDF", strScipt1, true);
                //}
            }
            else
    ...
    protected void SaveDocument(string strHTML, string ReferenceNumber, string FilePath, string FileName)
    {
        try
        {
            ...
            Procparam.Add("@DCNoteHeaderId", ReferenceNumber);
            dsPrintDetails = Utility.GetDataset("S3G_LAD_GETPrint_DrCrnote", Procparam);
            ...
            if (ddlType.SelectedValue == "1")
            {
                if (Convert.ToInt32(Stock_Hdr_Id) > 5064)
                {
                    if (Convert.ToString(dsPrintDetails.Tables[0].Rows[0]["Ori_Invoice_No"]).Trim() != string.Empty &&
                dsPrintDetails.Tables[0].Rows[0]["Ori_Invoice_No"].ToString().Contains("/LAS/"))
                    {
                        strHTML = PDFPageSetup.FunPubGetTemplateContent(intCompanyID, 3, 0, 65, Stock_Hdr_Id);
                    }
                    else if (Convert.ToString(dsPrintDetails.Tables[0].Rows[0]["Ori_Invoice_No"]).Trim() != string.Empty &&
                    dsPrintDetails.Tables[0].Rows[0]["Ori_Invoice_No"].ToString().Contains("/BTN/"))
                    {
                        strHTML = PDFPageSetup.FunPubGetTemplateContent(intCompanyID, 3, 0, 64, Stock_Hdr_Id);
                    }
                }
                else
                {
                    strHTML = PDFPageSetup.FunPubGetTemplateContent(intCompanyID, 3, 0, 72, Stock_Hdr_Id);
                }
                if (strHTML == "")
                {
                    Utility.FunShowAlertMsg(this, "Template Master not defined");
            ...
            if (Convert.ToInt32(Stock_Hdr_Id) > 15189)
            {
                listImagePath.Add(Server.MapPath("../Images/TemplateImages/" + CompanyId + "/CompanyLogo.png"));
            }
            else
            ...
            if (Convert.ToInt32(Stock_Hdr_Id) > 15377)
            {
            ...
            if (nDigi_Flag == 1)
            {
                FunPubSaveDocument(strHTML, FilePath, FileName, "P", FooterNote);
            ...
            SignedFile = Server.MapPath(".") + "\\PDF Files\\DCN\\Signed\\";
            ...
            {
                DownFile = FilePath + FileName + ".pdf";
                SignedFile = Server.MapPath(".") + "\\PDF Files\\DCN\\Signed\\" + Path.GetFileName(DownFile);
                System.IO.File.Copy(Server.MapPath(".") + "\\PDF Files\\" + FileName + ".pdf", SignedFile, true);
            }
        }
        catch (Exception ex)
        {
