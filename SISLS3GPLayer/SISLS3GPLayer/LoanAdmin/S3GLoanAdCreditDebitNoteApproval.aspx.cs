...
/// /// <Program Summary>
/// Last Created By		      :   Sathish R on Oct-2014
...
public partial class S3GLoanAdManualJournalApproval_Add : ApplyThemeForProject
{
    ...
    public void FunPubViewApprovedDetails()
    {
        ...
        dictDropdownListParam.Add("@Task_Type", "60");//Account Revision Specific APPROVAL SCREEN LOOKUPCODE IS 29
        ...
        if (dsapproval.Tables[1].Rows.Count > 0)
        {
            ...
            ddlMJVNO.SelectedText = dsapproval.Tables[1].Rows[0]["MANUAL_JOURNAL_VOUCHER_No"].ToString();
            ddlMJVNO.SelectedValue = dsapproval.Tables[1].Rows[0]["MANUAL_JOURNAL_VOUCHER_ID"].ToString();
            ddlMJVNO.ToolTip = dsapproval.Tables[1].Rows[0]["MANUAL_JOURNAL_VOUCHER_ID"].ToString();
            ddlMJVNO.Enabled = false;

            //ddllLineOfBusiness.FillDataTable(dsapproval.Tables[3], "LOB_ID", "LOB_Name");
            ddllLineOfBusiness.Items.Add(new ListItem(dsapproval.Tables[1].Rows[0]["LOB_Name"].ToString(), dsapproval.Tables[1].Rows[0]["LOB_ID"].ToString()));
            ddllLineOfBusiness.ToolTip = dsapproval.Tables[1].Rows[0]["LOB_Name"].ToString();
            ddllLineOfBusiness.Enabled = false;

           // ddlBranch.FillDataTable(dsapproval.Tables[3], "Location_ID", "Location_Name");
            ddlBranch.SelectedText = dsapproval.Tables[1].Rows[0]["Location_Name"].ToString();
            ddlBranch.SelectedValue = dsapproval.Tables[1].Rows[0]["Location_ID"].ToString();
            ddlBranch.ToolTip = dsapproval.Tables[1].Rows[0]["Location_Name"].ToString();
            ...
            hdnID.Value = "../LoanAdmin/S3GDebitCreditNote_Add.aspx?qsViewId=" + FormsAuthentication.Encrypt(Ticket) + "&qsMode=Q&Popup=yes";

            Ds = Utility.GetDataset(SPNames.S3G_LOANAD_GetApprovalCommonDetail, dictDropdownListParam);

            grvApprovalDetails.DataSource = dsapproval.Tables[1];
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (intErrorCode == 0)
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Insert", "alert('Debit Credit Note Approved successfully.');window.location.href='../LoanAdmin/S3gLoanAdTransLander.aspx?Code=DCAP';", true);
                ...
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Insert", "alert('Debit Credit Note Rejected successfully.');window.location.href='../LoanAdmin/S3gLoanAdTransLander.aspx?Code=DCAP';", true);
                }
                return;
            }
            ...
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Insert", "alert('Debit Credit Note details Added failed.');", true);
                return;
            }
            else
            {
                Utility.FunShowValidationMsg(this.Page, "DCN", intErrorCode, strErrMsg, false);
                return;
            }
        }
    ...
    {
        Response.Redirect("~/LoanAdmin/S3gLoanAdTransLander.aspx?Code=DCAP");
    }
    #endregion
    ...
    protected void ddlMJVNO_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlMJVNO.SelectedValue != "0")
        {
            ...
            dictDropdownListParam = new Dictionary<string, string>();
            dictDropdownListParam.Add("@Task_Type", "60");
            dictDropdownListParam.Add("@Company_ID", intCompanyID.ToString());
            dictDropdownListParam.Add("@Task_Number", ddlMJVNO.SelectedValue.ToString());
            dt = Utility.GetDefaultData("S3G_LOANAD_GetApprovalReqDetail", dictDropdownListParam);
            if (dt.Rows.Count > 0)
            {
                txtStatus.Text = dt.Rows[0]["Status"].ToString();
                txtMJVDate.Text = DateTime.Parse(dt.Rows[0]["DOC_DATE"].ToString(), CultureInfo.CurrentCulture.DateTimeFormat).ToString(strDateFormat);
                txtvaluedate.Text = DateTime.Parse(dt.Rows[0]["DOC_DATE"].ToString(), CultureInfo.CurrentCulture.DateTimeFormat).ToString(strDateFormat);
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (intVoucher_No > 0)
            {
                if (dsapproval.Tables[1].Rows.Count > 0)
                {
                    ...
                    ddlstatus.SelectedValue = dsapproval.Tables[1].Rows[intRow]["Status_ID"].ToString();
    ...
    protected void btnRevoke_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (grvApprovalDetails.Rows.Count > 0)
            {
                ...
                dictDropdownListParam = new Dictionary<string, string>();
                dictDropdownListParam.Add("@OPTIONS", "7");
                dictDropdownListParam.Add("@TASK_TYPE", "60");
                ...
                switch (ErrCode)
                {
                    case 0:
                        {
                            ScriptManager.RegisterStartupScript(this, this.GetType(), "Insert", "alert('DCN Approval details Revoked successfully.');window.location.href='../LoanAdmin/S3gLoanAdTransLander.aspx?Code=DCAP';", true);
                            return;
                        }
                    case 1:
                        {
