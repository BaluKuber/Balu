...
public partial class LegalRepossession_S3GLRRepossession : ApplyThemeForProject
{
    ...
    public static LegalRepossession_S3GLRRepossession obj_Page;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["qsViewId"] != null)
        {
            ...
            txtRepossessionDate.Attributes.Add("onblur", "fnDoDate(this,'" + txtRepossessionDate.ClientID + "','" + strDateFormat + "',false,  true');");
        }
    ...
    private void PopulateBranchList()
    {
        try
        {
            ...
            if (PageMode == PageModes.Create)
                Procparam.Add("@Is_Active", "1");
            //ddlBranch.BindDataTable("S3G_Get_Branch_List", Procparam, new string[] { "Branch_ID", "Branch" });
        }
        catch (Exception ex)
        {
            cvRepossession.ErrorMessage = "Unable to Load Branch";
            cvRepossession.IsValid = false;
            return;
        }
    ...
    private void FunPriClear()
    {
        try
        {
            ...
            //ddlGarageCode.ClearDropDownList();
            //ddlAgentCode.ClearDropDownList();
            ddlRepossessionDoneBy.SelectedIndex = -1;
            txtRepoExp.Text = "";
            txtCondition.Text = "";
    ...
    private void PopulateAgentDetails()
    {
        ...
        {
            cvRepossession.ErrorMessage = "Unable to Loas Agent/Employee Details";
            cvRepossession.IsValid = false;
            return;
        }
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
         ...
         //   objRow.IS_Active = Convert.ToBoolean(ChkActive.Checked);
            ...
            objRow.Repossession_Place = txtPlace.Text;
            objRow.Asset_Condition = txtCondition.Text;
            if (rdoYes.Checked)
            ...
            objRow.Asset_Inventory_Details = txtAssetInventory.Text;
            objRow.Repossession_Expences = txtRepoExp.Text;
            objRow.Remarks = txtRemarks.Text;
            ...
            if (count == 0)
            {
                cvRepossession.ErrorMessage = "Select the Asset";
                cvRepossession.IsValid = false;
                return;
            }
            ...
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), strKey, "alert('Already Repossession for these Asset is Active');" + strRedirectPageView, true);
            }
    ...
    private void FunGetRepossionDetails(int repoid)
    {
        try
        {
            ...
            if (dsRepossession.Tables[0].Rows.Count > 0)
            {
                ...
                ddlBranch.SelectedValue = dtRow["Branch_ID"].ToString();
              //  PopulateLRNumber();
                ddlLRNNumber.Items.Add(dtRow["LRN_No"].ToString() + "-" + dtRow["Customer_Name"].ToString());
