...
#endregion
public partial class Fund_Management_S3G_FUNDMGT_TrancheCreation_View : ApplyThemeForProject
{

    #region Paging Config

    string strRedirectPage = "~/Fund Management/S3G_FundMgt_Tranche_add.aspx";
    int intNoofSearch = 3;
    string[] arrSortCol = new string[] { "Tranche_Name", "Funder_Name", "customer_name" ,""};
    string strProcName = "S3G_FUNDMGT_Tranche_paging";
    ...
    protected void grvPaging_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                // As per Ramesh , user wanted to update billing email alert even tranche is approved status
                //if (lblStatus.Text.ToString() == "Approved" || lblStatus.Text.ToString() == "Rejected")
                if (lblStatus.Text.ToString() == "Rejected")
                {
